...
class Laser:
    def __init__(self, screen):
        ...
        for i in range(len(self.laser_sprites)):
            self.laser_sprites[i] = pygame.transform.scale(
                self.laser_sprites[i], self.laser_size
            )
        ...
        # laser_mask
        self.laser_mask = pygame.mask.from_surface(
            self.laser_sprites[self.current_frames]
        )
    ...
    def create_laser(self):
        self.screen.blit(
            self.laser_sprites[self.current_frames], (self.pos_x, self.pos_y)
        )
