...
class Player:
    def __init__(self, color, size, velocity_x, pos_x, pos_y, health, screen):
        ...
        # player sprites
        self.player_idle = [
            pygame.image.load("asset/player_sprite/idle/00_idle.png"),
        ...
        self.walkRight = [
            pygame.image.load("asset/player_sprite/Walk/00_Walk.png"),
        ...
        for i in range(len(self.player_idle)):
            self.player_idle[i] = pygame.transform.scale(
                self.player_idle[i], self.player_size
            )

        for i in range(len(self.walkRight)):
            self.walkRight[i] = pygame.transform.scale(
                self.walkRight[i], self.player_size
            )

        for i in range(len(self.walkLeft)):
            self.walkLeft[i] = pygame.transform.scale(
                self.walkLeft[i], self.player_size
            )
    ...
    def update(self):
        ...
        if self.frame_count >= self.frame_delay:
            ...
            self.idle_mask = pygame.mask.from_surface(
                self.player_idle[self.current_frames]
            )
    ...
    def move(self, KEY):
        ...
        if KEY == "RIGHT":
            ...
            self.screen.blit(
                self.walkRight[self.current_frames], (self.pos_x, self.pos_y)
            )
            ...
            self.right = False
            self.screen.blit(
                self.walkLeft[self.current_frames], (self.pos_x, self.pos_y)
            )
    ...
    def idle(self):
        ...
        self.screen.blit(
            self.player_idle[self.current_frames], (self.pos_x, self.pos_y)
        )
